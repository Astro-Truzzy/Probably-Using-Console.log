...
export default function BlogCard({ post }: { post: PostSummary }) {
  return (
    <article className="log-card bg-[var(--bg-section)] rounded-xl shadow-sm border border-[var(--surface-border)] overflow-hidden">
      {post.cover && (
        <Link href={`/blog/${post.slug}`} className="block blog-card-cover">
          <Image
            src={post.cover}
            alt={post.title}
            width={640}
